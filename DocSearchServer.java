...
class Handler implements URLHandler {
    ...
    public String handleRequest(URI url) throws IOException {
        ...
        if (url.getPath().equals("/")) {
            ...
            if (parameters.length == 2 && parameters[0].equals("q")) {
                for (File f : paths) {
                    if (FileHelpers.readFile(f).contains(parameters[1])) {
                        foundPaths.add(f.toString());
                    }
                }
            } else if (parameters.length == 2 && parameters[0].equals("title")) {
                for (File f : paths) {
                    if (f.toString().contains(parameters[1])) {
                        ...
                        parameters[0].equals("title") &&
                        parameters[2].equals("q")) {
                for (File f : paths) {
                    if (f.toString().contains(parameters[1]) &&
                        FileHelpers.readFile(f).contains(parameters[3])) {
                        foundPaths.add(f.toString());
                    }
                }
            } else {
                return "Couldn't find query parameter q or title";
            }
